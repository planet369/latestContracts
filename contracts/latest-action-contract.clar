     ...
     (fund_info (unwrap! (map-get? fund-allocation { action_id: action_id,  fund_id: fund_id}) (err 21) ))
      ...
      (recipient tx-sender)
       ...
       (begin 
        ...
        (unwrap! (as-contract (contract-call? 'ST1XK1467FKTFQ6K4XHWFQR4XFEK3X7PCR5J464SR.ard-usd-01 transfer USD tx-sender recipient)) (err u21))
...
(define-map terms-indices {action_id: (string-utf8 256)} {index: uint}) ;; this will enable us to get length of each of the following actions 


(define-map terms_and_conditions {action_id: (string-utf8 256), index: uint} {term: (string-utf8 256), term_id: (string-utf8 256), votes: uint})
...
(define-public (add-terms (term (string-utf8 256)) (term_id (string-utf8 256)) (action_id (string-utf8 256)))
    ...
    (let (
        ...
        (action-term-index (map-get? terms-indices {action_id: action_id}))
     (a-index (get index action-term-index))
     (next-index (+ (default-to u0 a-index) u1))
    ) 
     (asserts! (is-eq tx-sender  owner) (err u21))
        (map-set terms_and_conditions { action_id: action_id,  index: next-index} { 
                                term: term, 
                                term_id: term_id, 
                                votes: u0, 
                                })
...
(define-public (vote-on-term ) 

)

(define-read-only (get-voted-terms) 

) ;; gets the votes ratio of donors over 51

(define-read-only (get-fund-allocation (uuid (string-utf8 256)) ) )

(define-read-only (get-fund-reciepts (uuid (string-utf8 256)) ) )
(define-read-only (get-donors-list (uuid (string-utf8 256)) ) )
